...
syn keyword vbOperator AddressOf And Eqv Imp In
...
syn keyword vbFunction Hour IIf IMEStatus IPmt InStr InStrRev Input InputB
...
syn keyword vbMethods AddFromFile AddFromGuid AddFromStrInStrRev ing
...
syn keyword vbKeyword As Binary Date Empty Error Friend Get
syn keyword vbKeyword Input Is Len Lock Me New Nothing Null On
...
syn keyword vbTypes Integer Long Object Single String Variant ByVal Byref
...
syn region  vbString		start=+"+  end=+"\|$+ contains=@Spell 
syn region  vbComment		start="\(^\|\s\)REM\s" end="$" contains=@Spell
syn region  vbComment		start="\(^\s*\| \)'"   end="$" contains=@Spell
syn region  vbComment		start="'"   end="$" contains=@Spell
